...
## 含参积分
设有函数 $f(u,x)$, 其中 $x\in(E,\mathcal A,\mu)$ 为测度空间, $u\in U$ 为度量空间. 我们定义 $F(u) = \int f(u,x) \,\mathrm{d}\mu(x)$
...
则函数 $F(u) = \int f(u,x) \,\mathrm{d}\mu(x)$ 是良定义的, 且在 $u_0$ 点连续.
...
则函数 $F(u) = \int f(u,x) \,\mathrm{d}\mu(x)$ 在 $u_0$ 点连续且可导, 导数为
$$F'(u_0) = \int \frac{\partial f}{\partial u}(u,x) \,\mathrm{d}\mu(x).$$
...
$$\int \varphi_\infty(x)\,\mathrm{d}\mu(x) = \lim_{n\to\infty}\int \varphi_n(x)\,\mathrm{d}\mu(x) = \lim_{n\to\infty}\frac{F(u_n)-F(u_0)}{u_n-u_0}$$
故 $F$ 在 $u_0$ 处可导, 导数为 $F'(u_0) = \int \varphi_\infty(x)\,\mathrm{d}\mu(x)$.

另一方面, 我们可以扩充 $\frac{\partial f}{\partial u}(u_0,x)$ 的定义域至整个 $E$ 且对 $\mu-p.p.$ 的 $x$, $\frac{\partial f}{\partial u}(u_0,x) = \varphi_\infty(x)$, 故
$$F'(u_0) = \int \varphi_{\infty}(x) \,\mathrm{d}\mu(x) = \int\frac{\partial f}{\partial u}(u_0,x)\,\mathrm{d}\mu(x).$$
